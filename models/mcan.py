...
class MHAtt(nn.Module):
    def __init__(self):
        ...
        self.linear_v = nn.Linear(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'], utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])
        self.linear_k = nn.Linear(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'], utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])
        self.linear_q = nn.Linear(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'], utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])
        self.linear_merge = nn.Linear(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'], utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])

        self.dropout = nn.Dropout(utils.MCAN_GQA_PARAMS['DROPOUT_R'])
    ...
    def forward(self, v, k, q, mask):
        ...
        v = self.linear_v(v).view(
            n_batches,
            -1,
            utils.MCAN_GQA_PARAMS['MULTI_HEAD'],
            int(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'] / utils.MCAN_GQA_PARAMS['MULTI_HEAD'])
        ).transpose(1, 2)
        ...
        atted = atted.transpose(1, 2).contiguous().view(
            n_batches,
            -1,
            utils.MCAN_GQA_PARAMS['HIDDEN_SIZE']
        )
...
class FFN(nn.Module):
    def __init__(self):
        ...
        self.mlp = MLP(
            in_size=utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'],
            mid_size=utils.MCAN_GQA_PARAMS['FF_SIZE'],
            out_size=utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'],
            dropout_r=utils.MCAN_GQA_PARAMS['DROPOUT_R'],
            use_relu=True
        )
...
class SA(nn.Module):
    def __init__(self):
        ...
        self.dropout1 = nn.Dropout(utils.MCAN_GQA_PARAMS['DROPOUT_R'])
        self.norm1 = LayerNorm(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])

        self.dropout2 = nn.Dropout(utils.MCAN_GQA_PARAMS['DROPOUT_R'])
        self.norm2 = LayerNorm(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])
...
class SGA(nn.Module):
    def __init__(self):
        ...
        self.dropout3 = nn.Dropout(utils.MCAN_GQA_PARAMS['DROPOUT_R'])
        self.norm3 = LayerNorm(utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])
...
class MCA_ED(nn.Module):
    def __init__(self):
        ...
        self.enc_list = nn.ModuleList([SA() for _ in range(utils.MCAN_GQA_PARAMS['LAYER'])])
        self.dec_list = nn.ModuleList([SGA() for _ in range(utils.MCAN_GQA_PARAMS['LAYER'])])
...
class AttFlat(nn.Module):
    def __init__(self):
        ...
        self.mlp = MLP(
            in_size=utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'],
            mid_size=utils.MCAN_GQA_PARAMS['FLAT_MLP_SIZE'],
            out_size=utils.MCAN_GQA_PARAMS['FLAT_GLIMPSES'],
            dropout_r=utils.MCAN_GQA_PARAMS['DROPOUT_R'],
            use_relu=True
        )

        self.linear_merge = nn.Linear(
            utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'] * utils.MCAN_GQA_PARAMS['FLAT_GLIMPSES'],
            utils.MCAN_GQA_PARAMS['FLAT_OUT_SIZE']
        )
    ...
    def forward(self, x, x_mask):
        ...
        att_list = []
        for i in range(utils.MCAN_GQA_PARAMS['FLAT_GLIMPSES']):
            att_list.append(
...
class MCANCustomized(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.connector = nn.Linear(self.encoder.out_dim, utils.MCAN_GQA_PARAMS['HIDDEN_SIZE'])
    ...
    def forward(self, ims, ques_ix, info_nce=False, pretrain=False):
        if pretrain:
            B = ims.size(0)
            logits = torch.zeros(B, utils.MCAN_GQA_PARAMS['ANSWER_SIZE']).to(ims)
            if info_nce:
